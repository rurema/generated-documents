...
それぞれについては [spec/control#rescue_modifier](../doc/spec=2fcontrol.md#rescue_modifier)、[spec/def#defined](../doc/spec=2fdef.md#defined)、
[spec/control#if_modifier](../doc/spec=2fcontrol.md#if_modifier)、[spec/control#unless_modifier](../doc/spec=2fcontrol.md#unless_modifier)、
[spec/control#while_modifier](../doc/spec=2fcontrol.md#while_modifier)、[spec/control#until_modifier](../doc/spec=2fcontrol.md#until_modifier) を参照してください。
...
- **再定義できる演算子(メソッド)**:
  ...
  ```text
      ...
      +  -  *  /    %   **   ~   +@  -@  []  []=  ` ! != !~
  ```
  これらの演算子式の定義方法については[spec/def#operator](../doc/spec=2fdef.md#operator)を参照してください。
...
- **属性参照**:
  ...
  属性は [Module#attr](../method/Module/i/attr.md) を使って同じように定義できます。
...
多重代入は複数の式または配列から同時に代入を行います。左辺の各式はそれぞれ代入可能でなければなりません。右辺の式が一つしか与えられなかった場合、式を評価した値は配列に変換されて、各要素が左辺のそれぞれの式に代入されます。左辺の要素の数よりも配列の要素の数の方が多い場合には、余った要素は無視されます。
配列の要素が足りない場合には対応する要素の無い左辺には
nil が代入されます。

左辺のいずれかの式の直前に * がついていると、対応する左辺のない余った要素が配列として代入されます。余った要素が無い時には空の配列が代入されます。* は左辺の最後の式の直前だけでなく、途中の式の直前につけることもできます。

なお、左辺の式が一つだけで `*` もついていない場合は多重代入とはみなされず、右辺の値(複数の式が `,` で並んでいれば配列に変換された値)
がそのまま単純代入されます。
...
条件式以外の場所では式1から式2までの範囲オブジェクトを返します。範囲オブジェクトは[Range](../class/Range.md)クラスのインスタンスです。 ... で生成された範囲オブジェクトは終端を含みません。
...
左辺の評価結果が偽であった場合には右辺を評価しその評価結果を返します。
