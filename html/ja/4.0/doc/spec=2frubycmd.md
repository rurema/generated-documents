...
ここで、option は後述の[cmd_option](#cmd_option)
のいずれかを指定します。-- は、オプション列の終りを明示するために使用できます。programfile は、Ruby スクリプトを記述したファイルです。これを省略したり`-` を指定した場合には標準入力を Ruby スクリプトとみなします。

programfile が `#!` で始まるファイルである場合、特殊な解釈が行われます。詳細は後述の[shebang](#shebang) を参照してください

argument に指定した文字列は組み込み定数 [Object::ARGV](../method/Object/c/ARGV.md) の初期値として設定されます。標準のシェルがワイルドカードを展開しない環境
([platform/Win32](../doc/platform=2fWin32.md))では、Ruby インタプリタが自前でワイルドカードを展開して
[Object::ARGV](../method/Object/c/ARGV.md) に設定します。この場合ワイルドカードとして
...
- **`--encoding ex[:in]`**:
  ...
  [Encoding.default_internal](../method/Encoding/s/default_internal.md) は nil になります。また、:エンコーディ
...
- **`-i[extension]`**:
  ...
  だけが残ります。ただし [platform/Win32](../doc/platform=2fWin32.md) では省略出来ません
  ([ruby-list:38066] 参照)。
...
- **`-l`**:
  ...
  gets
  で読み込まれた各行の最後に対して
  [String#chomp!](../method/String/i/chomp=21.md)を行います。
...
- **`-r feature`**:
  ...
  [Kernel?.require](../method/Kernel/m/require.md) します。
...
- **`-s`**:
  ...
  [Object::ARGV](../method/Object/c/ARGV.md) から取り除かれます。
...
- **`-W:category`**:
  ...
  ```
    ここで設定された値は [Warning.\[\]](../method/Warning/s/=5b=5d.md) で参照できます。
