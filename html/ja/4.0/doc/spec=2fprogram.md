...
Ruby の式には、さまざまな [spec/literal](../doc/spec=2fliteral.md)、[spec/variables](../doc/spec=2fvariables.md)、[spec/call](../doc/spec=2fcall.md)、それらの [spec/operator](../doc/spec=2foperator.md)、`if` や `while` などの [spec/control](../doc/spec=2fcontrol.md)、
[spec/def](../doc/spec=2fdef.md) があります。
...
ただし、`return`、`break`、`next` といったものは値を返しません。これらは評価された時点で制御が移ってしまいます。
...
メソッドの引数に指定できない式は、括弧によるグルーピングを行うことで普通の式として使用できます。
...
なお、`not` 演算子による式は、Ruby 3.3 までのデフォルトパーサ(parse.y)ではメソッドの引数にできませんが、Ruby 3.4 以降のデフォルトパーサ(Prism)では引数に指定できます。
