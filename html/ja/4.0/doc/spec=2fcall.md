...
ここでの「識別子」はメソッド名を表します。メソッド名には通常の識別子の他、識別子に `?` または `!` の続いたものや、再定義可能な演算子(例: `==`, `+`, `-` など)も使えます([spec/def#method](../doc/spec=2fdef.md#method)参照)。

メソッド呼び出し式はレシーバ(`.` の左側の式の値)のメソッドを呼び出します。レシーバが指定されない時は self のメソッドを呼び出します。

`.` と `::` とはほぼ同じ意味です。但し、定数を表す場合は、
`::` を使わなければいけません(例: [Math::PI](../method/Math/c/PI.md))。逆に、
...
`!` はメソッド名の一部です。同名の(`!` の無い)メソッドに比べて、より注意して使うべきメソッドであることを示します(例: tr と tr!)。破壊的なメソッドに使われることが多いですが、`!` が付いていないメソッドが破壊的でないとは限りません。
...
最後の引数の直前に & がついている場合、その引数で指定した手続きオブジェクト([Proc](../class/Proc.md))やメソッドオブジェクト([Method](../class/Method.md))がブロックとしてメソッドに渡されます。詳細は [block](#block) を参照してください。

メソッド呼び出しの際、private なメソッドは関数形式(レシーバを省略した形式)でしか呼び出すことができません。また protected なメソッドはそのメソッドを持つオブジェクトのメソッド定義式内でなければ呼び出せません。[spec/def#limit](../doc/spec=2fdef.md#limit) を参照して下さい。

ハッシュ式の解説([spec/literal#hash](../doc/spec=2fliteral.md#hash))にも書かれている通り、メソッド呼出の末尾にHashを渡す場合は {, } を省略できます。
これを用いることでキーワード引数を渡すことができます。
この Hash は[spec/def#method](../doc/spec=2fdef.md#method)で解説されているキーワード引数に分解されて引き渡されます。

メソッド呼び出しで `.` の代わりに `&.` を使うことができます。
`&.` は safe navigation operator と呼ばれます(「ぼっち演算子」と呼ばれることもあります)。
この形式でメソッドを呼びだそうとすると、レシーバが nil の場合は以下のように働きます。
...
```text title="文法"
method(arg1, arg2, ...)  do [`|' 式 ... [`;' 識別子 ... ] `|'] 式 ... end
method(arg1, arg2, ...) `{' [`|' 式 ... [`;' 識別子 ... ] `|'] 式 ... `}'
...
(ブロックと呼ばれる)を後ろに付けてメソッドを呼び出すと、そのメソッドの内部からブロックを評価できます。
...
ブロックパラメータの後に `;` で区切って識別子を並べると、それらは
[ブロックローカル変数](../doc/glossary.md#block-local-variable)になります。
ブロックローカル変数は、外側のスコープに同名のローカル変数があってもそれとは別の変数になり(シャドーイング)、外側の変数に影響を与えずに同じ名前を使えます。値はブロックの呼び出しごとに `nil` に初期化されます。

```ruby
x = 10
[1, 2, 3].each do |i; x|  # x はブロックローカル変数
  x = i * 10              # 外側の x とは別の変数
  p x                     # => 10, 20, 30
end
p x                       # => 10
...
それを実現するのが手続きオブジェクト([Proc](../class/Proc.md))です。
それをブロックとして渡すにはブロック付きメソッドの最後の引数として `&` で修飾した手続きオブジェクトを渡します。Proc の代わりにメソッドオブジェクト([Method](../class/Method.md))を渡すことも出来ます。この場合、そのメソッドを呼ぶ手続きオブジェクトが生成され渡されます。
...
[spec/control#break](../doc/spec=2fcontrol.md#break) により中断された場合は nil を返します。
...
引数をブロックパラメータとして渡してブロックを評価します。yield はイテレータを定義するために [spec/def#method](../doc/spec=2fdef.md#method) 内で使用します。
...
ブロックパラメータの代入は[spec/operator#multiassign](../doc/spec=2foperator.md#multiassign)と同じルールで行われます。
また yield を実行したメソッドにブロックが渡されていない
(ブロック付きメソッド呼び出しではない)時は例外 [LocalJumpError](../class/LocalJumpError.md) が発生します。

yield はブロック内で最後に評価した式の値を返します。また、
[spec/control#next](../doc/spec=2fcontrol.md#next) によりブロックの実行が中断された場合は nil
を返します。
...
下記はcallメソッドの糖衣構文です。
[Proc#call](../method/Proc/i/call.md)にも言及がありますが、[Proc](../class/Proc.md)以外のオブジェクトに対しても（callメソッドさえ定義されていれば）使えます。
