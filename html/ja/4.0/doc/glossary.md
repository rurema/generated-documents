...
- **Bignum**: {#bignum}
  ...
  クラスだった。Ruby 2.4 で Fixnum と共に [Integer](../class/Integer.md) に一本化された。
...
- **CSI 方式**: {#csi}
  ...
  与えられた文字列を単一の内部エンコーディングに変換するのではなく、さまざまな
...
- **dRuby**: {#druby}
  標準添付ライブラリー [drb](../library/drb.md) によって提供される分散オブジェクトプログラミングの
  ための仕組み。
...
- **Fixnum**: {#fixnum}
  ...
  そうでないものは Bignum のインスタンスであった。[Integer](../class/Integer.md) はこれらをまとめる
  抽象クラスだった。
...
- **アリティー**: {#arity}
- **arity**:
  メソッドやそれをオブジェクト化した [Method](../class/Method.md) オブジェクトの仮引数の数、および
  ブロックやそれをオブジェクト化した [Proc](../class/Proc.md) オブジェクトの
  ブロックパラメーターの数。
...
- **spaceship operator**:
  [Comparable](../class/Comparable.md) モジュールが利用する、二つのオブジェクトの順序関係を表す
  演算子 <=> の俗称。
...
- **空**: {#empty}
- **empty**:
  [Array](../class/Array.md) や [Hash](../class/Hash.md)、[Struct](../class/Struct.md) のような要素を持つクラスでは、要素が無いことを
  「空（から）」と呼ぶ。
...
- **コレクション**: {#collection}
- **collection**:
  配列、ハッシュ、集合（[Set](../class/Set.md) オブジェクト）など、
...
- **大域脱出**: {#non-local-exit}
- **non-local exit**:
  Ruby においては、例外の発生もしくは [Kernel?.throw](../method/Kernel/m/throw.md) によって
  処理の実行が中断されること。
  例外の場合は begin/rescue によって、throw の場合は
  [Kernel?.catch](../method/Kernel/m/catch.md) によって捕捉されるまで、制御構造やメソッド
  呼び出しの階層もすべて抜ける。
...
- **data type**:
  ...
  String、[Time](../class/Time.md) といった クラスには [Comparable](../class/Comparable.md) モジュール
...
- **block argument**:
  メソッドに与えられたブロックを [Proc](../class/Proc.md) オブジェクトとして受け取るため & 付きで
  記述された仮引数。
...
- **method**:
  ...
  メソッド自体はオブジェクトではないが、[Method](../class/Method.md) オブジェクトとして
  オブジェクト化できる。
...
- **rational number**:
  ...
  Ruby で「有理数」といった
  場合、[Rational](../class/Rational.md) クラスのインスタンスを指すことが多い。
...
- **reflection**:
  ...
  [Module#define_method](../method/Module/i/define_method.md) で動的にメソッドを定義したり、といったことが
  できる。
