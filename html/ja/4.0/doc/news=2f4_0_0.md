...
- [Array](../class/Array.md)
  - 新規メソッド
    - `array.reverse_each.find` のより効率的な代替として [Array#rfind](../method/Array/i/rfind.md) が追加されました。[feature:21678]
    - [Enumerable#find](../method/Enumerable/i/find.md) のより効率的なオーバーライドとして `Array#find` が追加されました。[feature:21678]

- [Binding](../class/Binding.md)
  - 変更されたメソッド
    - [Binding#local_variables](../method/Binding/i/local_variables.md) が番号指定パラメータを含まなくなりました。また、[Binding#local_variable_get](../method/Binding/i/local_variable_get.md)、[Binding#local_variable_set](../method/Binding/i/local_variable_set.md)、[Binding#local_variable_defined?](../method/Binding/i/local_variable_defined=3f.md) は番号指定パラメータを扱おうとすると拒否するようになりました。[bug:21049]
  - 新規メソッド
    - 番号指定パラメータと `it` パラメータにアクセスするための [Binding#implicit_parameters](../method/Binding/i/implicit_parameters.md)、[Binding#implicit_parameter_get](../method/Binding/i/implicit_parameter_get.md)、[Binding#implicit_parameter_defined?](../method/Binding/i/implicit_parameter_defined=3f.md) が追加されました。[bug:21049]

- [Enumerator](../class/Enumerator.md)
  - 変更されたメソッド
    - [Enumerator.produce](../method/Enumerator/s/produce.md) がオプションの `size` キーワード引数を受け取るようになりました。列挙子のサイズを指定するもので、整数、`Float::INFINITY`、呼び出し可能オブジェクト(ラムダなど)、またはサイズ不明を表す `nil` を指定できます。指定しなかった場合、サイズはデフォルトで `Float::INFINITY` になります。
...
- [Fiber](../class/Fiber.md)
  - 新規メソッド
    - [Kernel?.raise](../method/Kernel/m/raise.md) と同様の `cause:` 引数を [Fiber#raise](../method/Fiber/i/raise.md) でサポートしました。[feature:21360]
...
- `Fiber::Scheduler`
  - 新規メソッド
    ...
    - 非同期の [IO#close](../method/IO/i/close.md) のための `Fiber::Scheduler#io_close` フックが再導入されました。
    - IO の書き込みバッファをフラッシュする際に `Fiber::Scheduler#io_write` が呼び出されるようになりました。[bug:21789]

- [File](../class/File.md)
  - 新規メソッド
    - カーネルとファイルシステムが対応していれば、Linux でも [statx システムコール](man:statx(2linux))を介して [File::Stat#birthtime](../method/File=3a=3aStat/i/birthtime.md) が利用できるようになりました。[feature:21205]

- [IO](../class/IO.md)
  - 変更されたメソッド
    - [IO.select](../method/IO/s/select.md) がタイムアウト引数として Float::INFINITY を受け取れるようになりました。[feature:20610]
  - 非推奨だった、先頭に `|` を付けた IO のクラスメソッドによるプロセス生成が削除されました。[feature:19630]

- [Kernel](../class/Kernel.md)
  - 変更されたメソッド
    - [`Kernel#inspect`](../method/Object/i/inspect.md)が `instance_variables_to_inspect` というメソッドの存在を確認するようになり、これによって inspect の出力に含めるインスタンス変数を制御できるようになりました。
      ...
      [feature:21219]
  - 非推奨だった、先頭に `|` を付けた [Kernel?.open](../method/Kernel/m/open.md) によるプロセス生成が削除されました。[feature:19630]
...
- [Proc](../class/Proc.md)
  - 変更されたメソッド
    - [Proc#parameters](../method/Proc/i/parameters.md) が、匿名の必須引数の場合と表示が一貫するように、匿名のオプション引数を `[:opt, nil]` ではなく `[:opt]` として表示するようになりました。[bug:20974]

- [Ractor](../class/Ractor.md)
  - Ractor 間で通信するための新しい同期機構として [Ractor::Port](../class/Ractor=3a=3aPort.md) クラスが追加されました。[feature:21262]
    ...
    - [Ractor::Port#receive](../method/Ractor=3a=3aPort/i/receive.md)
    - [Ractor::Port#send](../method/Ractor=3a=3aPort/i/send.md) (別名 [Ractor::Port#<<](../method/Ractor=3a=3aPort/i/=3c=3c.md))
    - [Ractor::Port#close](../method/Ractor=3a=3aPort/i/close.md)
    - [Ractor::Port#closed?](../method/Ractor=3a=3aPort/i/closed=3f.md)

  - この結果、`Ractor.yield` と `Ractor#take` は削除されました。

  - [Ractor#join](../method/Ractor/i/join.md) と [Ractor#value](../method/Ractor/i/value.md) が、Ractor の終了を待つために追加されました。それぞれ [Thread#join](../method/Thread/i/join.md)、[Thread#value](../method/Thread/i/value.md) に相当するものです。

  - [Ractor#monitor](../method/Ractor/i/monitor.md) と [Ractor#unmonitor](../method/Ractor/i/unmonitor.md) が、[Ractor#join](../method/Ractor/i/join.md) を内部的に実装するために使われる低レベルのインターフェイスとして追加されました。

  - [Ractor.select](../method/Ractor/s/select.md) は Ractor と Port のみを受け付けるようになりました。Ractor を渡した場合は、いずれかの Ractor が終了した時点で返るようになります。

  - [Ractor#default_port](../method/Ractor/i/default_port.md) が追加されました。各 Ractor はデフォルトの port を持っており、[Ractor#send](../method/Ractor/i/send.md) や [Ractor.receive](../method/Ractor/s/receive.md) に使われます。

  - `Ractor#close_incoming` と `Ractor#close_outgoing` は削除されました。

  - shareable な Proc やラムダを作成するための [Ractor.shareable_proc](../method/Ractor/s/shareable_proc.md) と [Ractor.shareable_lambda](../method/Ractor/s/shareable_lambda.md) が導入されました。[feature:21550] [feature:21557]

- [Range](../class/Range.md)
  - 変更されたメソッド
    - `Range#to_set` が、終端のない(endless)範囲での問題を防ぐためのサイズチェックを行うようになりました。[bug:21654]
    - [Range#overlap?](../method/Range/i/overlap=3f.md) が無限(範囲に境界がない)範囲を正しく扱うようになりました。[bug:21185]
    - beginless な整数範囲における [Range#max](../method/Range/i/max.md) の挙動の不具合が修正されました。[bug:21174] [bug:21175]
...
- [Set](../class/Set.md)
  - [Set](../class/Set.md) が、autoload される stdlib のクラスではなく、コアクラスになりました。[feature:21216]
  - [Set#inspect](../method/Set/i/inspect.md) が、リテラル配列と同様のよりシンプルな表示を使うようになりました(例: `#<Set: {1, 2, 3}>` ではなく `Set[1, 2, 3]`)。[feature:21389]
  - `Set#to_set` と [Enumerable#to_set](../method/Enumerable/i/to_set.md) に引数を渡すことは非推奨になりました。[feature:21390]

- [Socket](../class/Socket.md)
  - [Socket.tcp](../method/Socket/s/tcp.md) と [TCPSocket.new](../method/TCPSocket/s/new.md) が、最初の接続のタイムアウトを指定するための open_timeout キーワード引数を受け取るようになりました。[feature:21347]
  - [TCPSocket.new](../method/TCPSocket/s/new.md) でユーザー指定のタイムアウトが発生した場合、状況によって [Errno::ETIMEDOUT](../class/Errno=3a=3aETIMEDOUT.md) または [IO::TimeoutError](../class/IO=3a=3aTimeoutError.md) のいずれかが送出されることがありましたが、常に [IO::TimeoutError](../class/IO=3a=3aTimeoutError.md) が送出されるように統一されました。(なお [Socket.tcp](../method/Socket/s/tcp.md) では、同様の状況で依然として [Errno::ETIMEDOUT](../class/Errno=3a=3aETIMEDOUT.md) が発生する場合があり、また両方のケースにおいて OS レベルでタイムアウトが発生した場合には [Errno::ETIMEDOUT](../class/Errno=3a=3aETIMEDOUT.md) が発生することがあります。)
...
- [String](../class/String.md)
  ...
  - [String#strip](../method/String/i/strip.md)、[String#strip!](../method/String/i/strip=21.md)、[String#lstrip](../method/String/i/lstrip.md)、[String#lstrip!](../method/String/i/lstrip=21.md)、[String#rstrip](../method/String/i/rstrip.md)、[String#rstrip!](../method/String/i/rstrip=21.md) が `*selectors` 引数を受け取れるように拡張されました。[feature:21552]

- [Thread](../class/Thread.md)
  - 新規メソッド
    - [Kernel?.raise](../method/Kernel/m/raise.md) と同様の `cause:` 引数を [Thread#raise](../method/Thread/i/raise.md) でサポートしました。[feature:21360]
...
- [Ractor::Port](../class/Ractor=3a=3aPort.md) の追加に伴い、以下のメソッドが Ractor から削除されました。[feature:21262]
  - `Ractor.yield`
  - `Ractor#take`
  - `Ractor#close_incoming`
  - `Ractor#close_outgoing`

- [ObjectSpace?._id2ref](../method/ObjectSpace/m/_id2ref.md) が非推奨になりました。[feature:15408]
...
- 「wrong number of arguments」の [ArgumentError](../class/ArgumentError.md) のバックトレースに、レシーバのクラス名またはモジュール名が含まれるようになりました(例えば単なる `bar` の中ではなく `Foo#bar` の中、というように)。[bug:21698]
...
- [cgi](../library/cgi.md) ライブラリが default gems から削除されました。現在は以下のメソッドのために `cgi/escape` のみを提供します。[feature:21258]
  - [CGI.escape](../method/CGI/s/escape.md) と [CGI.unescape](../method/CGI/s/unescape.md)
  - [CGI.escapeHTML](../method/CGI/s/escapeHTML.md) と [CGI.unescapeHTML](../method/CGI/s/unescapeHTML.md)
  - [CGI.escapeURIComponent](../method/CGI/s/escapeURIComponent.md) と [CGI.unescapeURIComponent](../method/CGI/s/unescapeURIComponent.md)
  - [CGI.escapeElement](../method/CGI/s/escapeElement.md) と [CGI.unescapeElement](../method/CGI/s/unescapeElement.md)

- [Set](../class/Set.md) が stdlib からコアクラスに移動したことに伴い、`set/sorted_set.rb` が削除され、`SortedSet` は autoload される定数ではなくなりました。`SortedSet` を使うには sorted_set gem をインストールして `require 'sorted_set'` してください。[feature:21287]
...
- Set
  - [Set](../class/Set.md) のための C API が追加されました。以下のメソッドがサポートされています。[feature:21459]
    - `rb_set_foreach`
    - `rb_set_new`
    - `rb_set_new_capa`
    - `rb_set_lookup`
...
- [Class#new](../method/Class/i/new.md) (例: `Object.new`)が全てのケースでより高速になりましたが、特にキーワード引数を渡す場合に顕著です。この変更は YJIT と ZJIT にも統合されています。[feature:21254]
- サイズプールごとの GC ヒープがそれぞれ独立して拡張するようになり、一部のプールにのみ長命なオブジェクトが存在する場合のメモリ使用量が削減されました。
- 大きなオブジェクトのページに対する GC のスイープが高速化されました。
- 「Generic ivar」オブジェクト([String](../class/String.md)、[Array](../class/Array.md)、`TypedData` など)が、より高速なインスタンス変数アクセスのために新しい内部的な「fields」オブジェクトを使うようになりました。
- GC が、最初に使われるまで内部的な id2ref テーブルを保持しないようになり、object_id の確保と GC のスイープが高速になりました。
- [Class](../class/Class.md) や [Module](../class/Module.md) オブジェクトでの `object_id` と `hash` が高速化されました。
- より大きな bignum の `Integer` が、可変長アロケーション(variable width allocation)を使って埋め込まれたままになれるようになりました。
- [Random](../class/Random.md)、[Enumerator::Product](../class/Enumerator=3a=3aProduct.md)、[Enumerator::Chain](../class/Enumerator=3a=3aChain.md)、[Addrinfo](../class/Addrinfo.md)、[StringScanner](../class/StringScanner.md) や一部の内部オブジェクトが write-barrier protected になり、GC のオーバーヘッドが削減されました。
...
- バグ修正と安定性
  ...
  - GC の後に [TracePoint](../class/TracePoint.md) が動作しなくなる問題を修正しました。[bug:19112]
...
- [RubyVM::YJIT.runtime_stats](../method/RubyVM=3a=3aYJIT/s/runtime_stats.md)
...
- [RubyVM::YJIT.enable](../method/RubyVM=3a=3aYJIT/s/enable.md) に `mem_size:` と `call_threshold:` オプションが追加されました。
