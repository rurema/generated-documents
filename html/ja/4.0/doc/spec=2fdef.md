...
クラスを定義します。クラス名は大文字で始まる識別子です(「大文字」の定義は[spec/variables#const](../doc/spec=2fvariables.md#const)を参照してください)。

rescue/ensure 節を指定し、例外処理ができます。
例外処理については[spec/control#begin](../doc/spec=2fcontrol.md#begin)参照。

クラス定義は、識別子で指定した定数へのクラスの代入になります
(Ruby では、クラスもオブジェクトの一つで [Class](../class/Class.md)クラスのインスタンスです)。
...
[limit](#limit)のデフォルトが異なること以外にトップレベルとの違いはありません。クラス定義式中には任意の式を書くことができクラス定義の際に実行されます。
...
モジュールを定義します。モジュール名は大文字で始まる識別子です(「大文字」の定義は[spec/variables#const](../doc/spec=2fvariables.md#const)を参照してください)。
...
など [spec/operator](../doc/spec=2foperator.md) を参照)も指定できます([operator](#operator)参照)。
...
（デフォルト値を指定する2つの引数の間に、デフォルト値を指定しない引数があってはいけません）
...
最後の仮引数の直前に & があるとこのメソッドに与えられているブロックが手続きオブジェクト([Proc](../class/Proc.md))としてこの引数に格納されます。これは、イテレータを定義する方法の一つです。イテレータを定義する代表的な方法は
yield を呼び出すことです。
他に [Proc.new](../method/Proc/s/new.md)/[Kernel?.proc](../method/Kernel/m/proc.md) を使う方法などもあります。
...
またメソッド実行時の例外を捕捉するために begin 式と同様のrescue, else, ensure 節を指定できます。
例外処理については[spec/control#begin](../doc/spec=2fcontrol.md#begin)参照。

メソッド定義式は、メソッド名を [Symbol](../class/Symbol.md) にしたオブジェクトを返します。

- **SEE** <https://magazine.rubyist.net/articles/0041/0041-200Special-kwarg.html>

#### 演算子式の定義 {#operator}

[spec/operator](../doc/spec=2foperator.md)において、「再定義可能な演算子」に分類された演算子の実装はメソッドなので、定義することが可能です。
...
は未定義メソッドの呼び出しで例外 [NameError](../class/NameError.md) を発生させます。

### 特異メソッド定義 {#singleton_method}
...
これは、クラスの特異クラスが、スーパークラスの特異クラスのサブクラスとして作られるためです。この仕組みによって、クラスの特異メソッド(クラスメソッド)がサブクラスにも継承されます。
...
p B.singleton_class.superclass == A.singleton_class # => true
```

特異メソッド定義式は、メソッド名を [Symbol](../class/Symbol.md) にしたオブジェクトを返します。
...
extend については、[Object#extend](../method/Object/i/extend.md) を参照してください。
...
デフォルトでは def 式がクラス定義の外(トップレベル)にあれば private、クラス定義の中にあれば public に定義します。これは [Module#public](../method/Module/i/public.md)、[Module#private](../method/Module/i/private.md)、
[Module#protected](../method/Module/i/protected.md) を用いて変更できます。ただし [Object#initialize](../method/Object/i/initialize.md) という名前のメソッドと
[Object#initialize_copy](../method/Object/i/initialize_copy.md) という名前のメソッドは定義する場所に関係なく常に private になります。
...
正確には、private には関数を定義する目的があるが、呼び出し制限の目的でも(ここに挙げた制限があるにもかかわらず)
...
メソッドあるいはグローバル変数に別名をつけます。メソッド名には、通常の識別子の他、`?` や `!` の付いたものや[operator](#operator)で説明する再定義可能な演算子も含むメソッド名そのものか、
[spec/literal#symbol](../doc/spec=2fliteral.md#symbol) を指定します(obj.method のような式を書くことはできません)。alias の引数はメソッド呼び出し等の一切の評価は行われません。

メソッドの定義内で別名を付けるには[Module](../class/Module.md)クラスのメソッド
[Module#alias_method](../method/Module/i/alias_method.md) を利用して下さい。
...
([spec/variables#builtin](../doc/spec=2fvariables.md#builtin) を参照)を再定義すると動作に支障を来す場合があります。
...
メソッドの定義を取り消します。メソッド名には、通常の識別子の他、`?` や `!` の付いたものや[operator](#operator)で説明する再定義可能な演算子も含むメソッド名そのものか [spec/literal#symbol](../doc/spec=2fliteral.md#symbol) を指定します(obj.method のような式を書くことはできません)。undef
の引数はメソッド呼び出し等の一切の評価は行われません。

メソッドの定義内で定義を取り消すには[Module](../class/Module.md)クラスのメソッド [Module#undef_method](../method/Module/i/undef_method.md) を利用して下さい。

undef のより正確な動作は、メソッド名とメソッド定義との関係を取り除き、そのメソッド名を特殊な定義と関連づけます。この状態のメソッドの呼び出しはたとえスーパークラスに同名のメソッドがあっても例外 [NameError](../class/NameError.md) を発生させます。
(一方、メソッド [Module#remove_method](../method/Module/i/remove_method.md) は、関係を取り除くだけです。この違いは重要です)。

alias による別名定義と undef による定義取り消しによってクラスのインタフェースをスーパークラスと独立に変更できます。ただし、メソッドが self にメッセージを送っている場合もあるので、よく注意しないと既存のメソッドが動作しなくなる可能性があります。
...
式が定義されていなければ、nil を返します。定義されていれば式の種別を表す文字列を返します。

定義されていないメソッド、undef されたメソッド、[Module#remove_method](../method/Module/i/remove_method.md)
...
[Kernel?.block_given?](../method/Kernel/m/block_given=3f.md) と同様にメソッドがブロック付きで呼ばれたかを判断する方法になります。
