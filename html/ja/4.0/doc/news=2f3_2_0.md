...
## 言語仕様の変更
  ...
  - 可変長パラメータ (*args など) を受け取るメソッドで、 foo(*args) を通して
    ...
    他の委譲方法へ移行することが容易になります。以前は、受け取り側のメソッドが *args を
...
## 組み込みクラスの更新(注目すべきもののみ)
  ...
  - Class
    - 新規メソッド
      - Class#attached_object はレシーバが特異クラスであるオブジェクトを返します。レシーバが特異クラスでない場合は TypeError を発生させます。 [feature:12084]
        ...
        ```ruby title="Class#attached_object"
        class Foo; end

        p Foo.singleton_class.attached_object      # => Foo
        p Foo.new.singleton_class.attached_object  # => #<Foo:0x000000010491a370>
  ...
  - Data
    - 新規クラス
      - 単純かつ不変な値オブジェクトを表現するための新たなコアクラス Data が追加されました。 Data は Struct によく似ており、部分的に実装を共有しています。しかし、より限定的かつ少ないAPIとなっています。 [feature:16122]
        ...
        p weight.with(amount: 40)                    # => #<data Measure amount=40, unit="kg">
        p weight.amount                              # => 50
  ...
  - RubyVM::AbstractSyntaxTree
    - 変更されたメソッド
      - parse, parse_file, of へ error_tolerant オプションが追加されました。 [feature:19013]
        ...
        - 4. インデントをもとに end をキーワードとして扱います。
          ...
          ```ruby title="RubyVM::AbstractSyntaxTree.parse"
    ...
    - 変更されたメソッド
      - parse, parse_file, of へ keep_tokensオプションが追加されました。 RubyVM::AbstractSyntaxTree::Node に #tokens と #all_tokens が追加されました。 [feature:19070]
        ...
        p root.tokens # => [[0, :tIDENTIFIER, "x", [1, 0, 1, 1]], [1, :tSP, " ", [1, 1, 1, 2]], ...]
        p root.tokens.map{_1[2]}.join # => "x = 1 + 2"
        ```
  ...
  - Struct
    - 変更されたメソッド
      - Structクラスは Struct.new の実行時に keyword_init: true をつけなくてもキーワード引数によって初期化できるようになりました。 [feature:16806]
        ...
        ```ruby title="Struct.new"
        Post = Struct.new(:id, :name)
        p Post.new(1, "hello") # => #<struct Post id=1, name="hello">
        # Ruby 3.2から以下のコードも keyword_init: true をつけなくても動作する
        p Post.new(id: 1, name: "hello") # => #<struct Post id=1, name="hello">
        ```
  ...
  - GC
    - 変更されたメソッド
      - GC.latest_gc_info を介して need_major_gc を公開しました。 <https://github.com/ruby/ruby/pull/6791>

  - ObjectSpace
    - 変更されたメソッド
      - ObjectSpace.dump_all は形状もダンプするようになりました。 <https://github.com/ruby/ruby/pull/6868>
...
default gems と bundled gems の詳細については GitHub Releases of Logger(<https://github.com/ruby/logger/releases>) または changelog ファイルを参照してください。

## サポートプラットフォームの追加

  - WebAssembly/WASI が追加されました。 詳細は wasm/README.md(<https://github.com/ruby/ruby/blob/master/wasm/README.md>) と ruby.wasm(<https://github.com/ruby/ruby.wasm>) を参照してください。 [feature:18462]

## 互換性に関する変更

  - 現在、 [String#to_c](../method/String/i/to_c.md) では、アンダースコアの連続をComplex文字列の末尾として扱っていました。 [bug:19087]
  - [ENV.dup](../method/ENV/s/dup.md) と同様に [ENV.clone](../method/ENV/s/clone.md) も TypeError を発生するようになりました。 [bug:17767]
...
### クラス/モジュールの定義時の定数探索
  ...
  - オブジェクトクラスの直下にclass/module文でクラス/モジュールを定義する際、 [Module#include](../method/Module/i/include.md) で定義された同名のクラス/モジュールが既にある場合、Ruby 3.1以前ではオープンクラスとして処理されていました。Ruby 3.2以降では、代わりに新しいクラスが定義されます。 [feature:18832]

## 標準添付ライブラリの互換性

  - [Psych](../class/Psych.md) に同梱していた libyaml のソースコードは削除されました。また、Fiddleもlibffiのソースをバンドルしなくなりました。ユーザーは、apt、yum、brew などのパッケージマネージャを使って、自分で libyaml/libffi ライブラリをインストールする必要があります。

  - [Psych](../class/Psych.md) と fiddle には特定バージョンの libyaml や libffi のソースコードを静的リンクするための機能が追加されました。libyaml-0.2.5 をリンクしてビルドする場合は以下のように実行します。
  ...
  - CGI::Cookie の Cookie name/path/domain 文字をチェックします。 <https://www.ruby-lang.org/en/news/2022/11/22/http-response-splitting-in-cgi-cve-2021-33621/>

  - URI.parse が host に nil の代わりに空の文字列を返します。 <https://hackerone.com/reports/156615>
...
## 実装の改善
  ...
  - Variable Width Allocation(<https://shopify.engineering/ruby-variable-width-allocation>) がデフォルトで有効になりました。 [feature:18239]
