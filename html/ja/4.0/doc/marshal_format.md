...
'I' で始まるデータ構造になります。
[marshal_format#instance_variable](../doc/marshal_format.md#instance_variable)も参照してください。

```ruby title="例 2: インスタンス変数あり"
class Foo < Array # (or String, Regexp, Hash)
  def initialize(obj)
    @foo = false
    super(obj)
...
([marshal_format#Object](../doc/marshal_format.md#Object) 参照)
...
オプションは、[Regexp#options](../method/Regexp/i/options.md)の結果 + 漢字コードのフラグ値です。
...
```ruby title="例 2: クラス/モジュールのインスタンス変数は dump されない"
module Bar
  @bar = 1
end
...
```ruby title="例 3: クラス変数は dump されない"
module Baz
  @@baz = 1
  def self.baz
    @@baz
...
([marshal_format#class_module](../doc/marshal_format.md#class_module) 参照)
