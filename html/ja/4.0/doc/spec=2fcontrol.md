...
RubyはC言語やPerlから引き継いだ制御構造を持ちますが、その他に[spec/call#block](../doc/spec=2fcall.md#block)という制御構造の抽象化を援助する機能があります。ブロック付きメソッド呼び出しは繰り返しを始めとする制御構造をクラス設計者が定義する事が出来るものです。
...
パターンマッチ構文については[spec/pattern_matching](../doc/spec=2fpattern_matching.md)で説明しています。
...
また、引数を伴った break により while 修飾した式の戻り値をその値にすることもできます。
...
rescue 節以外で retry が用いられた場合には例外 [SyntaxError](../class/SyntaxError.md) が発生します。
...
```ruby title="例"
raise "you lose"  # ~> RuntimeError: you lose
...
第二の形式では、引数が文字列であった場合、その文字列をメッセージとする [RuntimeError](../class/RuntimeError.md) 例外を発生させます。引数が例外オブジェクトであった場合にはその例外を発生させます。第三の形式では第一引数で指定された例外を、第二引数をメッセージとして発生させます。第四の形式の第三引数は
[m:$@]または
[Kernel?.caller](../method/Kernel/m/caller.md)で得られるスタック情報で、例外が発生した場所を示します。

発生した例外は後述の begin 式の rescue 節で捕らえることができます。
その場合 rescue error_type => var の形式を使えば例外オブジェクトを得られます。このオブジェクトは組み込み変数 [m:$!] でも得られます。また例外が発生したソースコード上の位置は変数 [m:$@] に格納されます。

[Kernel?.raise](../method/Kernel/m/raise.md) は Ruby の予約語ではなく、[Kernel](../class/Kernel.md) モジュールで定義されている関数的メソッドです。
...
発生した例外は [m:$!] を使って参照できます。また、指定されていれば変数 evar にも $! と同様に発生した例外が格納されます。
...
error_type が省略された時は [StandardError](../class/StandardError.md) のサブクラスである全ての例外を捕捉します。Rubyの組み込み例外は([SystemExit](../class/SystemExit.md) や
[Interrupt](../class/Interrupt.md) のような脱出を目的としたものを除いて)
[StandardError](../class/StandardError.md) のサブクラスです。

例外クラスのクラス階層については
[_builtin](../library/_builtin.md)
を参照してください。

rescue では error_type は通常の引数と同じように評価され、そのいずれかが一致すれば本体が実行されます。error_type を評価した値がクラスやモジュールでない場合には例外 [TypeError](../class/TypeError.md) が発生します。
...
[spec/def#class](../doc/spec=2fdef.md#class)、[spec/def#module](../doc/spec=2fdef.md#module)、[spec/def#method](../doc/spec=2fdef.md#method)
などの定義文では、それぞれ
...
(つまり、[StandardError](../class/StandardError.md) 例外クラスのサブクラスだけしか捕捉できません)
...
BEGINはトップレベル以外では書けません。全て [SyntaxError](../class/SyntaxError.md)になります。
...
[spec/terminate](../doc/spec=2fterminate.md)を参照してください。
...
END ブロックは一つの記述につき最初の一回のみ有効です。たとえば以下のようにループの中で実行しても複数の END ブロックが登録されるわけではありません。そのような目的には [Kernel?.at_exit](../method/Kernel/m/at_exit.md) を使います。
...
意図的にこのようなことを行いたい場合は [Kernel?.at_exit](../method/Kernel/m/at_exit.md) を使います。
...
END や [Kernel?.at_exit](../method/Kernel/m/at_exit.md) で登録した後処理を取り消すことはできません。
