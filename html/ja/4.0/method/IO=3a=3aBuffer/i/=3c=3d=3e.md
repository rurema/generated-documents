...
[Comparable](../../../class/Comparable.md) を include しているため、`<` や `==` などの比較演算子も使えます。

- **param** `other` -- 比較対象のバッファを [IO::Buffer](../../../class/IO=3a=3aBuffer.md) で指定します。

- **raise** `TypeError` -- other が [IO::Buffer](../../../class/IO=3a=3aBuffer.md) でない場合に発生します。
