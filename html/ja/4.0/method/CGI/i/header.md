...
HTTP ヘッダを options に従って生成します。 [CGI#out](../../../method/CGI/i/out.md) と違い、標準出力には出力しません。
[CGI#out](../../../method/CGI/i/out.md) を使わずに自力で HTML を出力したい場合などに使います。
...
- **`expires`**:
  送信するコンテンツの有効期限を [Time](../../../class/Time.md) のインスタンスで指定します。
  Expires ヘッダに対応します。
- **`cookie`**:
  クッキーとして文字列か [CGI::Cookie](../../../class/CGI=3a=3aCookie.md) のインスタンス、またはそれらの配列かハッシュを指定します。
...
- **param** `options` -- [Hash](../../../class/Hash.md) か文字列で HTTP ヘッダを生成するための情報を指定します。
