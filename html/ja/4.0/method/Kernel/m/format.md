...
- **SEE** [Kernel?.printf](../../../method/Kernel/m/printf.md),[Time#strftime](../../../method/Time/i/strftime.md),[Date.strptime](../../../method/Date/s/strptime.md)
...
幅と精度を同じ値にすれば、どのような引数に対してもその長さだけの出力を行うことになります。
...
- **`s`**:
  ...
  引数が [String](../../../class/String.md) オブジェクトでなければ to_s メソッドにより文字列化
  したものを引数として扱います。

- **`p`**:

  [Object#inspect](../../../method/Object/i/inspect.md) の結果を出力します。
  ```ruby
  p sprintf("%s", /e+/)  # => "(?-mix:e+)"
  p sprintf("%p", /e+/)  # => "/e+/"
  ```
...
- **`i`**:
  ...
  引数が整数でなければ関数 [Kernel?.Integer](../../../method/Kernel/m/Integer.md) と同じ規則で整数に
  変換されます。
...
- **`A`**:
  ...
  a, A は指数表現の16進数("-h.hhh±pd") で数値を出力します。ただし、[Float::INFINITY](../../../method/Float/c/INFINITY.md)、[Float::NAN](../../../method/Float/c/NAN.md) はそれぞれ 'Inf'、'NaN' になります。詳しくは [ruby-dev:40650] を参照してください。
