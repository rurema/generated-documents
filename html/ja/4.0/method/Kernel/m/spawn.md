...
- **SEE** [Kernel?.system](../../../method/Kernel/m/system.md),[Kernel?.exec](../../../method/Kernel/m/exec.md)
...
env に Hash を渡すことで、[man:exec(2)] で子プロセス内でファイルを実行する前に環境変数を変更できます。
...
親プロセスは [Process?.waitpid](../../../method/Process/m/waitpid.md) で子プロセスの終了を待ち合わせるかもしくは [Process?.detach](../../../method/Process/m/detach.md) で子プロセスを切り離すかしてください。
...
- **`:exception`**:
  [Kernel?.system](../../../method/Kernel/m/system.md) のみで指定できます。
...
Hash の値(親プロセス側)には以下のいずれかが指定できます。
  ...
  - [リダイレクト先のファイル名文字列]、配列の要素にすることで
    [File::Constants::RDONLY](../../../method/File=3a=3aConstants/c/RDONLY.md) でファイルを開いてリダイレクトします。
...
ファイルデスクリプタを表すためには、以下が利用できます。
  ...
  - [IO](../../../class/IO.md) [IO#fileno](../../../method/IO/i/fileno.md) で表されるファイルデスクリプタ
...
詳しくは [Process?.setrlimit](../../../method/Process/m/setrlimit.md) を見てください。
このオプションには 整数 or 整数2つの配列、を渡すことができます。
それぞれ [Process?.setrlimit](../../../method/Process/m/setrlimit.md) の引数が2個、3個の場合に対応します。
...
これはファイルデスクリプタを直接指定するのと異なるということに注意してください。
例えば、
...
これを用いて、[IO.popen](../../../method/IO/s/popen.md) で、子プロセスの
...
標準的ファイルデスクリプタ(0,1,2)は :close で明示的に閉じない限り、このオプションの影響を受けません。
...
これを利用して spawn を [IO.popen](../../../method/IO/s/popen.md) のように使うことができます。
...
(Unix 系統の環境)のみです。
例えば Windows では元々子プロセスにはファイルデスクリプタ 0 1 2 のみ渡されるので :close_others や :close オプションは意味を持ちません。
