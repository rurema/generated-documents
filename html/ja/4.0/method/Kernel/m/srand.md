...
[Kernel?.rand](../../../method/Kernel/m/rand.md) や [Random.rand](../../../method/Random/s/rand.md) で使用される擬似乱数生成器の種を設定し、古い種を返します。

seed に整数を指定するとその絶対値を乱数の種に設定します。
それ以外の値を指定した場合は seed.to_int が指定されたものとして扱います。
seed に既知の値を与えると、以前の [Kernel?.rand](../../../method/Kernel/m/rand.md) の値を再現できます。
