...
指定された文字列 `str` に対して位置 `pos` から `self` が表す正規表現によるマッチングを行います。マッチした場合には結果を [MatchData](../../../class/MatchData.md) オブジェクトで返します。
...
ブロックを渡すと、マッチした場合に限り [MatchData](../../../class/MatchData.md) オブジェクトがブロック引数に渡されて実行されます。
