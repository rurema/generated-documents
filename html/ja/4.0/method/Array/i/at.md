...
- **param** `range` -- 生成したい部分配列の範囲を [Range](../../../class/Range.md) オブジェクトで指定します。
             range の begin や end の値が負の時には末尾からのインデックスとみなします。末尾の要素が -1 番目になります。
...
- **param** `start` -- 生成したい部分配列の先頭のインデックスを整数で指定します。
             start の値が負の時には末尾からのインデックスとみなします。
             末尾の要素が -1 番目になります。
             整数以外のオブジェクトを指定した場合は to_int メソッドによる暗黙の型変換を試みます。
