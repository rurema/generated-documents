...
- **`misc`**:
  ...
  以下の要素から構成される [Hash](../../../class/Hash.md) オブジェクト。

  :arg_size： メソッド、ブロックが取る引数の総数(1 つもない場合は 0)。

  :local_size： ローカル変数の総数 + 1。

  :stack_max： スタックの深さ。([SystemStackError](../../../class/SystemStackError.md) を検出するために使用)
...
- **`locals`**:
  ...
  全ての引数名、ローカル変数名からなる [Symbol](../../../class/Symbol.md) の配列。
