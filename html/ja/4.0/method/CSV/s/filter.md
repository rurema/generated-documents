...
与えられたブロックに一行ずつ渡されます。ブロックに渡された行は必要であれば変更できます。ブロックの評価後に行を全て output に書き込みます。

- **param** `input` -- [String](../../../class/String.md) か [IO](../../../class/IO.md) のインスタンスを指定します。
             デフォルトは [ARGF](../../../class/ARGF.md) です。

- **param** `output` -- [String](../../../class/String.md) か [IO](../../../class/IO.md) のインスタンスを指定します。
              デフォルトは [m:$stdout] です。
...
```ruby title="例: input, output は初期値"
...
```ruby title="例: input, output を指定する"
require "csv"
content = <<EOS
