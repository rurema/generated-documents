...
- **`:format_executable`**:
  ...
  インストールされている ruby が ruby19 という名前の場合、foo_exec という名前の実行ファイルは foo_exec19 という名前でインストールされます。
- **`:security_policy`**:
  特定のセキュリティポリシーを使用します。詳細は [Gem::Security](../../../class/Gem=3a=3aSecurity.md) を参照してください。
- **`:wrappers`**:
