...
### def on(short, desc = "") {|v| ... }        -> self
### def on(long, desc = "") {|v| ... }         -> self
### def on(short, long, desc = "") {|v| ... }  -> self
...
### def on(short, pat = /.*/, desc = "") {|v| ...}        -> self
### def on(long, pat = /.*/, desc = "") {|v| ...}         -> self
### def on(short, long, pat = /.*/, desc = "") {|v| ...}  -> self

オプションを取り扱うためのブロックを自身に登録します。
ブロックはコマンドラインのパース時に、オプションが指定されていれば呼ばれます。

pat にはオプションの引数に許すパターンを表す正規表現で与えます。
コマンドに与えられた引数がパターンにマッチしない場合、例外 OptionParser::InvalidArgument が parse 実行時に投げられます。
...
### def on(short, klass = String, desc = "") {|v| ...}        -> self
### def on(long, klass = String, desc = "") {|v| ...}         -> self
### def on(short, long, klass = String, desc = "") {|v| ...}  -> self

オプションを取り扱うためのブロックを自身に登録します。
ブロックはコマンドラインのパース時に、オプションが指定されていれば呼ばれます。

klass にはクラスを与えます。どのようなクラスを受け付けるかは、以下の「デフォルトで利用可能な引数クラス」を参照して下さい。
OptionParser.accept や OptionParser#accept によって、受け付けるクラスを増やすことができます。登録されていないクラスが指定された場合、例外 [ArgumentError](../../../class/ArgumentError.md) を投げます。
また、登録されたクラスであっても引数が変換できないものである場合、例外
[OptionParser::InvalidArgument](../../../class/OptionParser=3a=3aInvalidArgument.md) を投げます。
...
- **`String`**:
  ...
  [OptionParser::InvalidArgument](../../../class/OptionParser=3a=3aInvalidArgument.md) が発生します。
...
### def on(short, *rest) {|v| ...}        -> self
### def on(long, *rest) {|v| ...}         -> self
### def on(short, long, *rest) {|v| ...}  -> self

オプションを取り扱うためのブロックを自身に登録します。
ブロックはコマンドラインのパース時に、オプションが指定されていれば呼ばれます。

コマンドに与えられた引数が配列やハッシュに含まれない場合、例外
[OptionParser::InvalidArgument](../../../class/OptionParser=3a=3aInvalidArgument.md) が [OptionParser#parse](../../../method/OptionParser/i/parse.md) 実行時に発生します。
