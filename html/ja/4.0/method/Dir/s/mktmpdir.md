...
[FileUtils?.remove_entry](../../../method/FileUtils/m/remove_entry.md) を用いて削除し、ブロックの値をかえします。
...
- **param** `tmpdir` -- nil の場合は [Dir.tmpdir](../../../method/Dir/s/tmpdir.md) を使用します。
...
- **raise** `ArgumentError` -- tmpdirが全てのユーザから書き込み可能かつ、sticky
                     ビットが立っていない場合に発生します。作成する一時ディレクトリを安全に削除できないためです。
                     アプリケーションは一時ディレクトリを他のユーザから書き込める権限に変更すべきではありません。
