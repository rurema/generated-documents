...
- **`:ssl`**:
  ...
  [OpenSSL::SSL::SSLContext#set_params](../../../method/OpenSSL=3a=3aSSL=3a=3aSSLContext/i/set_params.md) にそのままパラメータとして渡されます。
...
- **`:implicit_ftps`**:
  ...
  デフォルトは false です。:ssl が偽のときに true を指定すると [ArgumentError](../../../class/ArgumentError.md) が発生します。
- **`:username`**:
  ログインに使うユーザ名を指定します。
- **`:password`**:
  ログインに使うパスワードを指定します。
...
- **`:passive`**:
  ...
  (既定値は [Net::FTP.default_passive=](../../../method/Net=3a=3aFTP/s/default_passive=3d.md) で変更できます)。
- **`:open_timeout`**:
...
- **SEE** [Net::FTP.open](../../../method/Net=3a=3aFTP/s/open.md)
