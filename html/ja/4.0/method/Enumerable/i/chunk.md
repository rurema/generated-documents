...
ただし、ブロックの評価値が nil、:_separator、:_alone の場合は特別扱いされます(詳細は後述)。

返り値の [Enumerator](../../../class/Enumerator.md) は各チャンクのブロック評価値と各チャンクの要素を持つ配列のペアを各要素とします。
...
このファイルは、Linux や macOS などで、ソートされた英語（など）の単語のリストを改行で区切って収めたものです。
...
以下の例では「Foo#bar」という形式の行が連続している場合のみチャンク化し、それ以外は素通しします。
