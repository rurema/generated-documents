...
FasterCSV は Ruby 1.8 に標準添付されている CSV ライブラリの置き換えとして開発されました。
...
- このパーサは m17n に対応しています。[CSV](../class/CSV.md) も参照してください
- このライブラリはより厳しいパーサを持っているので、問題のあるデータに対して [CSV::MalformedCSVError](../class/CSV=3a=3aMalformedCSVError.md) を投げます
...
- [CSV.open](../method/CSV/s/open.md) はより Ruby らしくなりました
- [CSV](../class/CSV.md) オブジェクトは [IO](../class/IO.md) の多くのメソッドをサポートするようになりました
- 文字列や IO のようなオブジェクトを読み書きするためにラップする [CSV.new](../method/CSV/s/new.md) メソッドが追加されました
- [CSV.generate](../method/CSV/s/generate.md) は古いものとは異なります
- 部分読み出しはもうサポートしていません。読み込みは行単位で行います
- パフォーマンスのため、インスタンスメソッドでセパレータを上書き出来なくなりました。
   [CSV.new](../method/CSV/s/new.md) でセットするようにしてください。

### CSV とは

CSV ライブラリは [RFC:4180] から直接とられたかなり厳しい定義を維持します。
一ヶ所だけ定義を緩和することでこのライブラリを使いやすくしています。[CSV](../class/CSV.md) はすべての有効な CSV ファイルをパースします。
