...
- Cで書かれた拡張ライブラリを含めたければ、[rdoc/parser/c](../library/rdoc=2fparser=2fc.md) を参照してください。
- コメント部で使えるマークアップについて知りたければ、[rdoc/markup](../library/rdoc=2fmarkup.md) を参照してください。
- RDoc をライブラリとして使いたければ、[RDoc::RDoc](../class/RDoc=3a=3aRDoc.md) を参照してください。
- テキスト部を HTML に変換する部分をライブラリとして使いたければ、[RDoc::Markup](../class/RDoc=3a=3aMarkup.md) を参照してください。
...
ドキュメントを読む人に取って便利なように、生成されるドキュメントのインデックスページに中心的なファイルに書かれている内容を含めることができます。
例えば、Rdoc そのもののドキュメントを生成する場合は、以下のようにタイプします。

```console
$ rdoc --main rdoc/rdoc.rb
```

RDoc が生成するドキュメントのコメント部で使える様々なマークアップの方法は以下の [markup](#markup) の項に書かれています。
...
- **`--force-update`**:
  ...
  Ruby 1.9.2 以下では指定しなかった場合は有効になりません。Ruby 1.9.2 以降は指定
  しなかった場合でも有効になります。
...
ただし、以下のオプションはユーザの指定するオプションや [RDoc](../class/RDoc.md) の通常の動作と干渉するため、保存する事ができません。
...
この場合は表形式となり、記述部(コロン2つの後のテキスト)は左揃えになります。この形式は本ドキュメントの末尾のほうの 'author' のところで使われています。
...
#### イタリック体、ボールド体、タイプライター体 {#italic_bold_typewriter}
...
単語というのは、アルファベットの大文字および小文字とアンダースコアーのみから構成された文字列です(よって日本語では使えません)。また、これらのマークアップ記号の前に「\」という文字を置くと、マークアップが抑制されます。上の表は以下のようにすれば作れます。
...
- **`:markup: type`**:
  ...
  ジェクト全体を設定してください
  ([rdoc#saved_options](../library/rdoc.md#saved_options) 参照)。
...
[rdoc/parser/ruby](../library/rdoc=2fparser=2fruby.md)、[rdoc/parser/c](../library/rdoc=2fparser=2fc.md) を参照してください。
