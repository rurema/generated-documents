...
dl と同等の機能を持ちますが、
dl は Ruby 2.0 以降 deprecated となり、Ruby 2.2 で削除されました。このライブラリを代わりに使います。
...
以下のように dlload を使ってライブラリをロードし、使用したいライブラリ関数に対して extern メソッドを呼んでラッパーメソッドを定義します。
...
fiddle においては、文字列/整数/[Fiddle::Pointer](../class/Fiddle=3a=3aPointer.md)をポインタとして扱うことができます。
文字列をポインタ引数として渡すと文字列のメモリ領域を指すポインタとして扱われます。
