...
また irb コマンドは [readline](../library/readline.md) ライブラリにも対応しています。
...
それぞれの設定値の詳細については、[IRB::Context](../class/IRB=3a=3aContext.md) を参照してください。

#### プロンプトのカスタマイズ {#customize_prompt}
...
フォーマット文字列では [Kernel?.printf](../method/Kernel/m/printf.md) のように「%」を用いた記法が使えます。
フォーマット文字列で使用可能な記法は以下の通りです。

- **`%N`**:
    起動しているコマンド名([IRB::Context#irb_name](../method/IRB=3a=3aContext/i/irb_name.md))
- **`%m`**:
...
irb では、起動時の irb インタプリタとは独立した環境を持つ「サブ irb」を任意の数だけ起動できます。
...
起動時のインタプリタでローカル変数 x を定義しましたが、「irb」でサブ irb を起動すると、ローカル変数 x が見えなくなっています。
これが「独立した環境」の意味です。

なお、「irb」コマンドによるサブ irb の起動(multi-irb)は非推奨となっており、将来の IRB 2.0.0 で削除される予定です。
今後は後述の「irb で使用可能なコマンド一覧」にある workspace 関連のコマンド
(`cws`、`pushws` など)の利用が推奨されています。

### サブ irb の設定 {#configure_sub_irb}

irb コマンド起動時のインタプリタの設定はコマンドラインオプションと IRB.conf の値で決まります。
それに対して、サブ irb インタプリタの設定は、各インタプリタの「conf」オブジェクトの値で決まります。
...
- **`irb_context`**:
  ...
  irb の現在の設定です。[IRB::Context](../class/IRB=3a=3aContext.md) オブジェクトです。
...
- **`irb_load(path, prev = nil)`**:
  ...
  [Kernel?.load](../method/Kernel/m/load.md) と異なり、path の内容を irb で一行ずつタイプしたか
...
- **`irb_require(path)`**:
  ...
  path に Ruby スクリプトを指定した場合は、[Kernel?.require](../method/Kernel/m/require.md) と異な
...
[IRB::Inspector.def_inspector](../method/IRB=3a=3aInspector/s/def_inspector.md) を参照してください。
