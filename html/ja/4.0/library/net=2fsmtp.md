...
ブロックを使うと File.open と同じように終端処理を自動的にやってくれるのでおすすめです。
...
セッションが開いたらあとは [Net::SMTP#send_message](../method/Net=3a=3aSMTP/i/send_message.md)
でメールを流しこむだけです。
...
メールを送ったら [Net::SMTP#finish](../method/Net=3a=3aSMTP/i/finish.md) を呼んでセッションを終了しなければいけません。
...
またブロック付きの [Net::SMTP.start](../method/Net=3a=3aSMTP/s/start.md), [Net::SMTP#start](../method/Net=3a=3aSMTP/i/start.md)
...
SMTP ではメールを送る側のホストの名前 (HELO ドメインと呼ぶ) を要求されます。HELO ドメインは [Net::SMTP.start](../method/Net=3a=3aSMTP/s/start.md), [Net::SMTP#start](../method/Net=3a=3aSMTP/i/start.md)
のキーワード引数 helo に指定します。
たいていの SMTP サーバはこの HELO ドメインによる認証はあまり真面目に行わないので (簡単に偽造できるからです)
...
[Net::SMTP](../class/Net=3a=3aSMTP.md) は PLAIN, LOGIN, CRAM MD5 の3つの方法で認証できます。
(認証については [RFC:2554], [RFC:2195] を参照してください)

認証するためには、[Net::SMTP.start](../method/Net=3a=3aSMTP/s/start.md) もしくは [Net::SMTP#start](../method/Net=3a=3aSMTP/i/start.md)
の引数に追加の引数を渡してください。
...
[Net::SMTP](../class/Net=3a=3aSMTP.md) は [RFC:3207] に基づいた STARTTLS を用いる方法、もしくは [RFC:8314] に基づいた方法
...
ただし、現在のメール送信の仕組みとして、あるサーバから別のサーバへの中継を行うことがあります。そこでの通信が認証されているか否か、暗号化されているか否かはこの仕組みの範囲外であり、なんらかの保証があるわけではないことに注意してください。メールそのものの暗号化や、メールを送る人、受け取る人を認証する必要がある場合は別の方法を考える必要があるでしょう。
...
サーバーが STARTTLS をサポートしているのに STARTTLS を使用したくない場合は [Net::SMTP#disable_starttls](../method/Net=3a=3aSMTP/i/disable_starttls.md) を使用します。
...
デフォルトではサーバー証明書の検証を行い、正当な証明書でない場合は [OpenSSL::SSL::SSLError](../class/OpenSSL=3a=3aSSL=3a=3aSSLError.md) 例外が発生します。
...
サーバー証明書に引数で指定したホスト名が含まれていなければ [OpenSSL::SSL::SSLError](../class/OpenSSL=3a=3aSSL=3a=3aSSLError.md) 例外が発生します。
