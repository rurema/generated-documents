...
[REXML::Parsers::SAX2Parser](../class/REXML=3a=3aParsers=3a=3aSAX2Parser.md) を用いてください。

パーサからはコールバックによってパースした情報を受け取ります。
[REXML::StreamListener](../class/REXML=3a=3aStreamListener.md) を include し、必要なメソッドをオーバーライドしたクラスのオブジェクトをコールバックオブジェクトとして [REXML::Parsers::StreamParser.new](../method/REXML=3a=3aParsers=3a=3aStreamParser/s/new.md)
に渡します。

[REXML::Parsers::StreamParser#parse](../method/REXML=3a=3aParsers=3a=3aStreamParser/i/parse.md) を呼び出すとパースが開始しその結果によってコールバックが呼び出されます。

コールバックされるメソッドは [REXML::StreamListener](../class/REXML=3a=3aStreamListener.md) になにもしないメソッドとして定義されています。どのようなコールバックがあるかはそちらを参照してください。

パーサはXML文書の各構成要素を前から順に処理し、その順にコールバックメソッドを呼び出します。順が前後することはありません。

### StreamParserの例 {#example}

この例では tag_start と text をオーバーライドして開始タグとテキストの情報を受け取れるようにしています。
...
StreamParser で処理したときに、どのコールバックメソッドがどのような引数で呼び出されるかを示すためのサンプルです。
実体参照は定義済みのものを除いては変換処理されていないことなどがわかります。
