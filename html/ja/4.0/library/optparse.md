...
- [subcmd](#subcmd)
- [argv](#argv)
- [hyphen_start_file](#hyphen_start_file)
...
[OptionParser#on](../method/OptionParser/i/on.md) メソッドの引数でオプションを定義し、引数が指定された時の処理をブロックで記述します。ブロックの引数にはオプションが指定されたことを示す true が渡されます([optionarg](#optionarg)の項も参照)。

[Enumerator#each](../method/Enumerator/i/each.md) などと違い、[OptionParser#on](../method/OptionParser/i/on.md)
メソッドが呼ばれた時点ではブロックは実行されません。あくまで登録されるだけです。[OptionParser#parse](../method/OptionParser/i/parse.md) が呼ばれた時に、コマンドラインにオプションが指定されていれば実行されます。
...
[OptionParser#parse!](../method/OptionParser/i/parse=21.md) により、コマンドライン(ARGV)の解析を行います。
[OptionParser#parse!](../method/OptionParser/i/parse=21.md) では、ARGV からオプションが取り除かれます。
これを避けるには [OptionParser#parse](../method/OptionParser/i/parse.md) を使います。
...
[OptionParser::InvalidOption](../class/OptionParser=3a=3aInvalidOption.md) が発生します。
...
[OptionParser](../class/OptionParser.md) 自体は、どのオプションが指定されたかを記憶しません。
...
この時ハッシュのキーとして利用される名前はロングオプションが定義されていればロングオプションの値を、ショートオプションのみの場合はショートオプションの値から、先頭の "-" を除いてシンボル化した値が使用されます。
...
[OptionParser#on](../method/OptionParser/i/on.md) メソッドのオプション定義で末尾に何かを書くと、そのオプションは引数を受け付けることの指定となります。
...
オプションの引数を省略すると例外 [OptionParser::MissingArgument](../class/OptionParser=3a=3aMissingArgument.md) が発生します。
...
ロングオプションは [OptionParser#on](../method/OptionParser/i/on.md) の引数に '--'で始まるオプションを指定します。
...
[OptionParser#on](../method/OptionParser/i/on.md) の引数にそのオプションの説明を加えると --help の出力に反映されます。
...
[OptionParser#order!](../method/OptionParser/i/order=21.md) がオプションではないコマンドの引数に出会うとそこでパースを中断することを利用しています。

#### ARGV の機能 {#argv}

optparse を require すると ARGV に [OptionParser::Arguable](../class/OptionParser=3a=3aArguable.md) の機能が加わります。以下の書き方ができるようになります。
[OptionParser::Arguable#getopts](../method/OptionParser=3a=3aArguable/i/getopts.md) はオプションを保持した Hash を返します。
...
これは POSIX.2 の [man:getopt(3)] に由来します。"--" 以降はすべてオプションではない引数として解釈されます。
