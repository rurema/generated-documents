...
[REXML::Parsers::StreamParser](../class/REXML=3a=3aParsers=3a=3aStreamParser.md) はパースした結果をコールバックによって受動的に受け取りますが、このパーサは [REXML::Parsers::PullParser#pull](../method/REXML=3a=3aParsers=3a=3aPullParser/i/pull.md)
によってパーサから結果をイベントという形で順に能動的に取り出します。
外部的にはこのクラスのオブジェクトはイベントのキューと見なせます。
pull はそのキューの先頭を取り出し、キューから取り除きます。

pull は [REXML::Parsers::PullEvent](../class/REXML=3a=3aParsers=3a=3aPullEvent.md) オブジェクトを返します。
このオブジェクトの
[REXML::Parsers::PullEvent#event_type](../method/REXML=3a=3aParsers=3a=3aPullEvent/i/event_type.md) で「開始タグ」「終了タグ」といったイベントの種類を取得します。
[REXML::Parsers::PullEvent#\[\]](../method/REXML=3a=3aParsers=3a=3aPullEvent/i/=5b=5d.md) でそのイベントのパラメータ
(例えば開始タグなら要素名と属性)を得ることができます。

### イベントの種類とパラメータ {#event_type}

[REXML::Parsers::PullEvent#event_type](../method/REXML=3a=3aParsers=3a=3aPullEvent/i/event_type.md) で得られるイベントの種類のシンボルを列挙しています。

これらのうちのいくつかは、
[REXML::Parsers::PullEvent#start_element?](../method/REXML=3a=3aParsers=3a=3aPullEvent/i/start_element=3f.md) などのメソッドで判定することが可能です。

- **start_element (要素名, 属性)**:
  XML要素の開始タグ。属性は { 属性名文字列 => 属性値文字列 } という [Hash](../class/Hash.md)
- **end_element (要素名)**:
  XML要素の終了タグ
- **text (正規化文字列, 非正規化文字列)**:
  テキストノード
...
- **start_doctype (ルート要素名, "SYSTEM" | "PUBLIC" | nil, システム識別子 | nil, 公開識別子 | nil)**:
  DTD 開始。判定は [REXML::Parsers::PullEvent#doctype?](../method/REXML=3a=3aParsers=3a=3aPullEvent/i/doctype=3f.md) メソッドで、
  start_doctype? ではない
- **`end_doctype ()`**:
  DTD 終了
- **attlistdecl (要素名, 属性名とデフォルト値, 宣言文字列)**:
  DTDの属性リスト宣言。属性名とデフォルト値 は { 属性名文字列 => デフォルト値文字列(なければnil) } という [Hash](../class/Hash.md)
- **elementdecl (宣言文字列)**:
  DTDの要素宣言
- **`entitydecl`**:
  DTDの実体宣言
