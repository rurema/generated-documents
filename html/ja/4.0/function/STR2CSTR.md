...
Ruby 1.7 以降では代わりに [StringValuePtr](../function/StringValuePtr.md) を使用します。こちらは、引数の参照先が暗黙の型変換の結果に置き換わるため変換結果が GC
されません。(Ruby 1.7 では、STR2CSTR() は、obsolete です)

[StringValue](../function/StringValue.md) は、引数が to_str による暗黙の型変換を期待する場合に使用します。
