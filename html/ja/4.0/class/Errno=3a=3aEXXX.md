...
例えば、[File.open](../method/File/s/open.md) の内部でシステムコール [man:open(2)] がエラーコード ENOENT を返すと、Rubyは例外 Errno::ENOENT を発生させます。
...
[Errno::EXXX](../class/Errno=3a=3aEXXX.md) (XXX はエラー番号を表す 3 桁の数字)
という名前の例外クラスが自動的に作成され、発生します。
(これは Ruby 内部でのみ行われ、 Ruby プログラム上で [SystemCallError.new](../method/SystemCallError/s/new.md) に一覧にないコードを渡してもクラスは作成されません)
...
- [new](../method/Errno=3a=3aEXXX/s/new.md)

## Constants

- [Errno](../method/Errno=3a=3aEXXX/c/Errno.md)
